...
class SayLavaMusic extends Manager {
  /**
   * 
   * @param {Client} client 
   */
  ...
  _loadEvents() {
    let count = 0;
    readdirSync("./src/Eventos/Lavalink/").forEach(file => {
      const event = require(`../Eventos/Lavalink/${file}`);
      let eventName = file.split(".")[0];
      this.on(eventName, event.bind(null, this.client));
    count++;
    });
    this.client.logger.log(`Os ${count} eventos do lavalink foram carregados com sucesso`, "event");
  }
}

module.exports = SayLavaMusic;